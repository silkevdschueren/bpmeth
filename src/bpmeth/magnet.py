...
class DipoleFromFieldmap:
    ...
    def __init__(
        ...
        if not h == 0:
            ...
            yFS = [0]
            sFS = np.arange(
                self.smin, self.smax, 0.001
            )  # Better a region that is a bit too large to have freedom :)

            self.fieldmap = self.fieldmap.calc_FS_coords(
                xFS, yFS, sFS, self.rho, self.phi, radius=radius
            )
            scalefactor = self.design_field / (
                self.fieldmap.integratedfield(3)[0] / self.l_magn
            )
    ...
    def fit_multipoles(self, plot=False, symmetric=True, guess=None):
        ...
        params_out_list, cov_out_list = self.fieldmap.fit_multipoles(
            self.shapefun,
            components=np.arange(1, self.order + 1, 1),
            design=1,
        ...
        integrated_field = 2 * np.trapz(
            get_derivative(0, self.nparams, func=self.shapefun, lambdify=True)(
                ss, *params_out_list[0]
            ),
            ss,
        )
        ...
        if not symmetric:
            params_in_list, cov_in_list = self.fieldmap.fit_multipoles(
                self.shapefun,
                components=np.arange(1, self.order + 1, 1),
                design=1,
                nparams=self.nparams,
            ...
            params_in_list[:, 0] = (
                params_in_list[:, 0] / self.Bfield / self.rho * scalefactor
            )
            warnings.warn(
                "be careful with asymmetric magnets, scalefactor might not be correct"
            )
        else:
    ...
    def calc_integrated_field(self, component):
        ...
        integrated_field = 2 * np.trapz(
            get_derivative(
                component - 1, self.nparams, func=self.shapefun, lambdify=True
            )(ss, *self.params_out_list[component - 1]),
            ss,
        )
        return integrated_field

    def create_fieldexpansion(self, plot=False, symmetric=True):
        params_in_list, params_out_list = self.fit_multipoles(
            plot=plot, symmetric=symmetric, guess=self.guess
        )
        ...
        for i in range(self.order):
            b_in.append(
                get_derivative(
                    i, nparams=self.nparams, func=self.shapefun, lambdify=False
                )(
                    -s - self.sedge if symmetric else (s - self.sedge),
                    *params_in_list[i],
                )
            )
            b_out.append(
                get_derivative(
                    i, nparams=self.nparams, func=self.shapefun, lambdify=False
                )(s - self.sedge, *params_out_list[i])
            )
    ...
    def create_Hamiltonian(self, plot=False, symmetric=True):
        ...
        # Make a Hamiltonian
        if self.h == 0:
            self.H_straight_in = Hamiltonian(
                -self.smin, 0, self.straight_in, s_start=self.smin
            )
            self.H_straight_out = Hamiltonian(
                self.smax, 0, self.straight_out, s_start=0
            )

        else:
            self.H_straight_in = Hamiltonian(
                self.smax - self.sedge, 0, self.straight_in, s_start=self.smin
            )
            self.H_bent_in = Hamiltonian(
                self.sedge, self.h, self.bent_in, s_start=-self.sedge
            )
            self.H_bent_out = Hamiltonian(self.sedge, self.h, self.bent_out, s_start=0)
            self.H_straight_out = Hamiltonian(
                self.smax - self.sedge, 0, self.straight_out, s_start=self.sedge
            )
    ...
    def track(self, particle, ivp_opt={}, plot=False):
        ...
        if self.h == 0:
            ...
            sol_sin = self.H_straight_in.track(
                particle, ivp_opt=ivp_opt, return_sol=True
            )
            ...
            sol_sout = self.H_straight_out.track(
                particle, ivp_opt=ivp_opt, return_sol=True
            )
            ...
            if plot:
                ...
                for i in range(len(particle.x)):
                    tlist = np.concatenate(
                        [sol_sin[i].t, sol_bin[i].t, sol_bout[i].t, sol_sout[i].t]
                    )
...
class MagnetFromFieldmap:
    ...
    def __init__(
        ...
        assert self.h != 0, (
            "This method is only implemented for curved magnets with h!=0"
        )
        if not h == 0:
            self.rho = 1 / h
            self.phi = l_magn * h
    ...
    def fit_multipoles(self, plot=False):
        ...
        segments, all_pols = self.fieldmap.fit_spline_multipoles(
            components=np.arange(1, self.order + 1, 1),
            ax=ax,
            step=self.step,
            smin=self.smin,
        ...
        bfuncs = [
            [poly_print(all_pols[i][j], x="s") for j in range(len(all_pols[i]))]
            for i in range(len(all_pols))
        ]
    ...
    def create_fieldexpansion(self, plot=False):
        ...
        for i in range(len(self.segments)):
            ...
            if (
                self.segments[i][0] < -self.sedge and self.segments[i][1] > self.smin
            ):  # Incoming drift
                expansion = FieldExpansion(b=bb, hs="0", nphi=self.nphi)
                expansions_in.append(expansion)
                segments_in.append(
                    [
                        max(self.segments[i][0], self.smin),
                        min(self.segments[i][1], -self.sedge),
                    ]
                )

            if (
                self.segments[i][1] > -self.sedge and self.segments[i][0] < self.sedge
            ):  # Body part
                expansion = FieldExpansion(b=bb, hs=f"{self.h}", nphi=self.nphi)
                expansions_body.append(expansion)
                segments_body.append(
                    [
                        max(self.segments[i][0], -self.sedge),
                        min(self.segments[i][1], self.sedge),
                    ]
                )

            if (
                self.segments[i][1] > self.sedge and self.segments[i][0] < self.smax
            ):  # Outgoing drift
                expansion = FieldExpansion(b=bb, hs="0", nphi=self.nphi)
                ...
                segments_out.append(
                    [
                        max(self.segments[i][0], self.sedge),
                        min(self.segments[i][1], self.smax),
                    ]
                )
    ...
    def create_Hamiltonian(self, plot=False):
        ...
        for i in range(len(self.expansions_in)):
            hamiltonians_in.append(
                Hamiltonian(
                    self.segments_in[i][1] - self.segments_in[i][0],
                    0,
                    self.expansions_in[i],
                    s_start=self.segments_in[i][0],
                )
            )
        for i in range(len(self.expansions_body)):
            hamiltonians_body.append(
                Hamiltonian(
                    self.segments_body[i][1] - self.segments_body[i][0],
                    self.h,
                    self.expansions_body[i],
                    s_start=self.segments_body[i][0],
                )
            )
        for i in range(len(self.expansions_out)):
            hamiltonians_out.append(
                Hamiltonian(
                    self.segments_out[i][1] - self.segments_out[i][0],
                    0,
                    self.expansions_out[i],
                    s_start=self.segments_out[i][0],
                )
            )
...
class FringeFromFint:
    def __init__(self, b0, fint, center=-0.5, gap=0.05):
        ...
        assert self.smax < 3.5, (
            "ELENA fringe field parameters are not suited for this choice"
        )
    ...
    def fit_fint(self):
        ...
        assert self.smax / scalefactor < 3.5, (
            "ELENA fringe field parameters are not suited for this choice"
        )
        return scalefactor
    ...
    def exitfringevalue(self, s, lambdify=False):
        ...
        return get_derivative(0, 6, spEnge, lambdify=lambdify)(
            (s - self.sedge) / self.scalefactor, *self.ELENA_enge_params
        )
...
class DipoleFromFint:
    ...
    def plot_components(self, ax=None):
        if ax is None:
            fig, ax = plt.subplots()
        if self.h == 0:
            self.straight_in.plot_components(
                smin=self.smin, smax=0, plot_bs=False, ax=ax
            )
            self.straight_out.plot_components(
                smin=0, smax=self.smax, plot_bs=False, ax=ax
            )
            integral = 0
            integral += np.trapz(
                [
                    self.straight_in.b[0].subs({self.straight_in.s: sval})
                    for sval in np.linspace(self.smin, 0, 300)
                ],
                np.linspace(self.smin, 0, 300),
            )
            integral += np.trapz(
                [
                    self.straight_out.b[0].subs({self.straight_out.s: sval})
                    for sval in np.linspace(0, self.smax, 300)
                ],
                np.linspace(0, self.smax, 300),
            )
            assert np.isclose(integral, self.b0 * self.l_magn, rtol=1e-4), (
                f"Integral of the field does not match the expected value! It is {integral} instead of {self.b0 * self.l_magn}!"
            )
        else:
            self.straight_in.plot_components(
                smin=self.smin, smax=-self.sedge, plot_bs=False, ax=ax
            )
            ...
            self.straight_out.plot_components(
                smin=self.sedge, smax=self.smax, plot_bs=False, ax=ax
            )
            integral = 0
            integral += np.trapz(
                [
                    self.straight_in.b[0].subs({self.straight_in.s: sval})
                    for sval in np.linspace(self.smin, -self.sedge, 300)
                ],
                np.linspace(self.smin, -self.sedge, 300),
            )
            integral += np.trapz(
                [
                    self.bent_in.b[0].subs({self.bent_in.s: sval})
                    for sval in np.linspace(-self.sedge, 0, 300)
                ],
                np.linspace(-self.sedge, 0, 300),
            )
            integral += np.trapz(
                [
                    self.bent_out.b[0].subs({self.bent_out.s: sval})
                    for sval in np.linspace(0, self.sedge, 300)
                ],
                np.linspace(0, self.sedge, 300),
            )
            integral += np.trapz(
                [
                    self.straight_out.b[0].subs({self.straight_out.s: sval})
                    for sval in np.linspace(self.sedge, self.smax, 300)
                ],
                np.linspace(self.sedge, self.smax, 300),
            )
            assert np.isclose(integral, self.b0 * self.l_magn, rtol=1e-4), (
                f"Integral of the field does not match the expected value! It is {integral} instead of {self.b0 * self.l_magn}!"
            )
    ...
    def track(self, particle, ivp_opt={}, amplify_plot=1):
        if self.h == 0:
            sol_in = self.H_straight_in.track(
                particle, s_span=[self.smin, 0], ivp_opt=ivp_opt
            )

            sol_out = self.H_straight_out.track(
                particle, s_span=[0, self.smax], ivp_opt=ivp_opt
            )
            ...
            fr3 = (
                ...
                .move_by(
                    [
                        (self.smin + self.sedge) * np.sin(self.angle / 2),
                        0,
                        (self.smin + self.sedge) * np.cos(self.angle / 2),
                    ]
                )
                .plot_zx(canvas=canvas)
            )
            fb = BendFrame(fr2, self.l_magn, self.angle).plot_zx(canvas=canvas)
            fr4 = fb.end
            fr5 = (
                ...
                .move_by(
                    [
                        -(self.smax - self.sedge) * np.sin(self.angle / 2),
                        0,
                        (self.smax - self.sedge) * np.cos(self.angle / 2),
                    ]
                )
                .plot_zx(canvas=canvas)
            )
            for i in range(len(particle.x)):
                fr3.plot_trajectory_zx(
                    sol_sin[i].t - self.smin,
                    sol_sin[i].y[0] * amplify_plot,
                    sol_sin[i].y[1],
                    canvas=canvas,
                )
                fb.plot_trajectory_zx(
                    sol_bin[i].t + self.sedge,
                    sol_bin[i].y[0] * amplify_plot,
                    sol_bin[i].y[1],
                    canvas=canvas,
                )
                fb.plot_trajectory_zx(
                    sol_bout[i].t + self.sedge,
                    sol_bout[i].y[0] * amplify_plot,
                    sol_bout[i].y[1],
                    canvas=canvas,
                )
                fr4.plot_trajectory_zx(
                    sol_sout[i].t - self.sedge,
                    sol_sout[i].y[0] * amplify_plot,
                    sol_sout[i].y[1],
                    canvas=canvas,
                )
